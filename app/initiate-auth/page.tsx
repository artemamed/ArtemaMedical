...
const NextStep: React.FC = () => {
  ...
  const handleAuthenticatePayer = async () => {
    try {
      ...
      console.log("Checkiing the response is ok or not", response);

      console.log("Checkiing the response DATA is ok or not", data);
      if (response.ok) {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gradient-to-r from-teal-600 to-teal-800 relative overflow-hidden">
      <div className="absolute w-96 h-96 bg-gradient-to-br from-yellow-500 to-teal-600 rounded-full blur-3xl opacity-50 transform scale-150 animate-pulse" />
      <div className="relative z-10 w-full max-w-lg p-10 bg-white rounded-3xl shadow-2xl border border-gray-200 transition-transform transform hover:scale-105 hover:shadow-3xl">
        <h1 className="text-5xl font-extrabold text-orange-700 text-center transition-all duration-200">Success!</h1>
        ...
        <button
          ...
          className={`mt-8 w-full py-4 rounded-xl text-xl font-bold text-white transition-all duration-300 transform ${loading ? 'bg-gray-500 cursor-not-allowed' : 'bg-teal-600 hover:bg-teal-800 shadow-lg hover:shadow-2xl'
            }`}
          disabled={loading}
        >
          {loading ? (
