export async function POST(req: Request) {
  ...
  try {
      ...
      return new Response(
        `<html>
                  <body style="text-align: center;">
                      ...
                      <button onclick="navigateToHome()" style="margin-top: 20px; padding: 10px 20px; font-size: 1em; background-color: #00897B; color: white; border: none; border-radius: 5px; cursor: pointer;">
            Back to Home
        </button>

        <script>
            function navigateToHome() {
                // If inside an iframe, force the parent window to navigate
    ...
    <body style="text-align: center;">
        ...
        <h1 style="font-size: 2em; color: #00897B;">Transaction Successful</h1>
        ...
        <script>
            function navigateToHome() {
                // If inside an iframe, force the parent window to navigate
                if (window.top !== window.self) {
                    window.top.location.href = "/cart/checkOut/orderComplete";
                } else {
                    window.location.href = "/cart/checkOut/orderComplete";
                }
            }
        </script>
    </body>
...
`,
      ...
      const failureReason =
        data.response.gatewayMessage || "Transaction failed";
      return new Response(
        `<html>
                  <body style="text-align: center;">
                      <svg width="80" height="80" viewBox="0 0 24 24" fill="red" xmlns="http://www.w3.org/2000/svg">
        ...
        <script>
            function navigateToHome() {
                // If inside an iframe, force the parent window to navigate
                if (window.top !== window.self) {
                    window.top.location.href = "/cart/checkOut";
                } else {
